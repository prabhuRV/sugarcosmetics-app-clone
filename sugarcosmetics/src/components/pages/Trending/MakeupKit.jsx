...
function MakeupKit() {
    ...
    useEffect(()=>{
        let getData= async()=>{
            try {
                let res= await fetch("http://localhost:8080/Sample");
                let info= await res.json();
                console.log(info)
                for(let i=0;i<20;i++)
                {
                  
                console.log("{"+"rating :"+info[0][i].averageRating+","+
                "price :"+info[0][i].product_price+","+
                "name :"+info[0][i].product_json.title+","+
                "image:"+"prabhu"+info[0][i].product_json.image.src+"prabhu"+"}")
            }
                setData([...info])
            } catch (error) {
                console.log(error)
